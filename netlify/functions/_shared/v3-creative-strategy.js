...
function conciseTheme(raw,contract,kind,facts){
 ...
 if(kind==="transformation")return value.split(/[.!?]/)[0].split(/\s+/).slice(0,8).join(" ");
 if(kind==="service")return value.split(/[.!?]/)[0].split(/\s+/).slice(0,8).join(" ")||contract.name;
 return value.split(/[.!?]/)[0].split(/\s+/).slice(0,9).join(" ")||"l’univers SDZ";
}
...
function genericVisualPlan(interpretation){
 ...
 if(interpretation.kind==="institutional")return {
  ...
  peoplePolicy:"aucune personne, aucun geste de soin et aucun matériel de prestation sauf demande explicite",
 ...
 if(interpretation.kind==="event")return {
  ...
  peoplePolicy:"n’inclure que les personnes nécessaires au rendez-vous décrit ; aucun praticien ni bénéficiaire inventé",
 ...
 if(interpretation.kind==="offer")return {
  ...
  peoplePolicy:"aucune personne ni rôle professionnel sauf nécessité explicite du sujet",
 ...
 if(interpretation.kind==="transformation")return {
  ...
  action:"évolution visible de la posture et de l’espace, sans geste de soin ni prestation inventés",
 ...
 return {
  ...
  action:"mise en scène concrète du sujet, sans geste de soin ni prestation inventés",
...
function inferSubjectBrief({subject,selectedRegisters=[],service,marketingObjective="Faire réserver",contract}={}){const rawSubject=clean(subject),serviceContract=contract||{name:service,bodyZones:"",recognition:"",requiredAction:""};const interpretation=editorialInterpretation(rawSubject,serviceContract);const genericPlan=serviceContract.generic?genericVisualPlan(interpretation):null;const explicit=/\b(?:temple|pavillon|jardin|galerie|arcade|forêt|montagne|cabinet|amphithéâtre)\b/i.exec(rawSubject)?.[0]||null;const requestedFocus=findRequestedFocus(rawSubject,serviceContract);const manifestation=`${interpretation.visualSubject}${requestedFocus?`, centrée sur ${requestedFocus}`:""}`;return Object.freeze({rawSubject,coreTheme:interpretation.theme,audienceProblem:interpretation.audienceIntent,physicalOrEmotionalManifestation:manifestation,transformationPromise:transformationFor(rawSubject,requestedFocus,serviceContract,interpretation),explicitSceneRequest:explicit,userConstraints:[],explicitTextRequest:null,selectedRegisters:[...selectedRegisters],relatedService:serviceContract.name,marketingObjective,requestedFocus,officialRecognition:genericPlan?.recognition||serviceContract.recognition,officialAction:genericPlan?.action||serviceContract.requiredAction,visualFocus:genericPlan?.focus||requestedFocus||serviceContract.recognition,visualPrimarySubject:genericPlan?.primarySubject||serviceContract.primarySubject,visualSecondarySubject:genericPlan?.secondarySubject||serviceContract.secondarySubject,peoplePolicy:genericPlan?.peoplePolicy||"personnes et rôles conformes au contrat métier",scenePolicy:genericPlan?.scenePolicy||"la prestation et son geste métier portent le message",visualProtectedAreas:genericPlan?.protectedAreas||[],editorialKind:interpretation.kind,editorialTitle:interpretation.title,editorialSubtitle:interpretation.subtitle,verifiedFacts:interpretation.facts,location:interpretation.location,address:interpretation.address,experienceSince:interpretation.year,noMedicalDiagnosis:true});}
...
module.exports={TEXT_MODES,GOLDEN_TARGET,CLAIM_RULES,inferSubjectBrief,editorialInterpretation,extractFacts,semanticLines,resolveTextMode,sanitizeClaims,buildPosterStrategy:buildResolvedPosterStrategy,buildLegacyProjection,relevanceScores,buildPostCopyStrategy,buildCampaignCreativeDirection};
