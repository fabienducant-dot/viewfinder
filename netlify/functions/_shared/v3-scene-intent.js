...
function resolveRegisters(subjectBrief={}){
 ...
 // The SDZ campaign default is luminous fantasy on every platform. Explicit realism wins.
 const realistic=/sans (?:effet )?fantastique|pas de fantastique|sans magie|sans surnaturel|strictement r[ée]aliste|photo documentaire|cabinet r[ée]el|photo r[ée]elle/i.test(raw);
...
function transformationFrom(subjectBrief={},mode="narrative_consequence",contract={}){
 ...
 if(mode==="narrative_consequence"){
  ...
  const repeatedMoment=clean(subjectBrief.dramaticMoment)===clean(subjectBrief.transformationPromise);
  return Object.freeze({before,moment:repeatedMoment?fallbackMoment:narrativeSafe(subjectBrief.dramaticMoment,fallbackMoment,exact),after:narrativeSafe(subjectBrief.transformationPromise,fallbackAfter,exact)});
 }
...
function environmentFor({subjectBrief,artDirection,registers,platform}){
 ...
 if(registers.fantastic)pieces.push(`Fantastique adulte et crédible, féerie spectaculaire immédiatement visible : ${art.fantasticPhenomenon||"un paysage impossible se déploie à une échelle saisissante"}. Relier ce phénomène au thème et à son évolution ; un reflet doré ou de la brume seuls ne suffisent pas. Matières photographiques, aucun symbole occulte.`);
 if(normalizePlatform(platform)==="Google Business")pieces.push("Visuel publicitaire métaphorique, jamais présenté comme une photographie du cabinet réel ; prestation et produit restent reconnaissables.");
...
function platformPolicyFor(platform){
 const p=normalizePlatform(platform);
 if(p==="Google Business")return Object.freeze({platform:p,brandingZone:"aucun texte intégré dans l’image brute",fantasyStrength:"full_unless_explicit_realism",composition:"publicité métaphorique 4:3, sujet lisible et trois plans lumineux"});
 if(p==="Story")return Object.freeze({platform:p,brandingZone:"réserver dans les 28 % inférieurs une bande de marque sombre, continue, texturée mais peu contrastée, surtout au centre ; aucun visage, main, produit, horizon, ouverture lumineuse, triangle, diagonale forte, reflet or intense ou élément narratif essentiel dans cette bande",fantasyStrength:"full_unless_explicit_realism",composition:"verticale 9:16, TROIS PLANS LISIBLES, scène dominante"});
 if(p==="Instagram Portrait"||p==="Instagram Square")return Object.freeze({platform:p,brandingZone:"réserver dans le tiers inférieur une zone sombre, continue et peu contrastée pour le lock-up ; aucune ouverture lumineuse, géométrie forte ou reflet or intense derrière la marque",fantasyStrength:"full_unless_explicit_realism",composition:"impact immédiat en miniature, sujet décentré et profondeur lisible"});
 if(p==="Facebook")return Object.freeze({platform:p,brandingZone:"zone éditoriale calme distincte du sujet",fantasyStrength:"full_unless_explicit_realism",composition:"lecture sociale claire, profondeur et respiration"});
 if(p==="Blog"||p==="Bannière")return Object.freeze({platform:p,brandingZone:"zone de respiration latérale ou supérieure selon le format",fantasyStrength:"full_unless_explicit_realism",composition:"composition horizontale éditoriale, sujet sur un tiers"});
 return Object.freeze({platform:p,brandingZone:"zone sûre calme",fantasyStrength:"full_unless_explicit_realism",composition:"composition éditoriale premium"});
}
...
function buildSceneIntent({contract,subjectBrief,artDirection,platform}){
 ...
 const validation=Object.freeze({requireDramaticMoment:mode==="narrative_consequence",requireTransformation:mode==="narrative_consequence",requireCinematicPoster:normalizePlatform(platform)!=="Google Business",requireThreePlanes:normalizePlatform(platform)!=="Google Business",requireArchitecture:registers.architectural&&normalizePlatform(platform)!=="Google Business",requireFantastic:registers.fantastic,requireLuminousGold:true,rejectGenericSpa:normalizePlatform(platform)!=="Google Business",requireProductFidelity:mode==="product_fidelity"||mode==="composite_fidelity",requireServiceGesture:mode==="demonstration"});
