...
const VERSION="4.2.0-scene-intent";
...
function withoutExact(value,exact){
  let text=clean(value),needle=clean(exact);
  if(!needle)return text;
  while(text.includes(needle))text=text.replace(needle,"le sujet demandé");
  return clean(text);
}
...
function transformationFrom(subjectBrief={},mode="narrative_consequence",contract={}){
  ...
  const focus=clean(subjectBrief.requestedFocus||"");
  if(mode==="narrative_consequence"){
    const before=[
      "Un état initial de contrainte, densité, saturation ou fermeture doit être perceptible par la posture, les volumes, les matières, la lumière ou l’espace",
      focus?`le focus visuel reste ${focus}`:"",
    ].filter(Boolean).join(" ; ")+".";
    ...
    return Object.freeze({before,moment:withoutExact(subjectBrief.dramaticMoment||fallbackMoment,exact),after:withoutExact(subjectBrief.transformationPromise||fallbackAfter,exact)});
  }
  const rawBefore=subjectBrief.physicalOrEmotionalManifestation||subjectBrief.subjectVisualDirective||contract.recognition||"une situation crédible et immédiatement lisible";
  const rawMoment=subjectBrief.dramaticMoment||"Un instant décisif et crédible où le sens de la scène devient évident.";
  const rawAfter=subjectBrief.transformationPromise||"Une expérience visuellement cohérente, sans promesse médicale.";
...
function peoplePolicyFor(contract,mode,subjectBrief={}){
  ...
  if(contract.name==="Massage Enfant")return mode==="narrative_consequence"
    ?"Un enfant entièrement vêtu et son parent peuvent incarner une situation quotidienne digne et rassurante. Aucun soin, aucun déshabillage, aucun contact corporel n’est montré."
...
function environmentFor({subjectBrief,artDirection,registers,platform}){
  ...
  const art=artDirection?.artistic||{},pieces=[];
...
function buildProviderPrompt({contract,subjectBrief,platform,mode,registers,transformation,peoplePolicy,environment,platformPolicy}){
  ...
  const narrativeRole=mode==="narrative_consequence"
    ...
    :mode==="local_credibility"?"La lisibilité et la crédibilité locale priment sur le spectaculaire.":"La prestation ou le produit peut être visible uniquement parce que ce mode exige une preuve réelle.";
  const lines=[
    "SCÈNE / INTENTION",`Demande exacte : ${exact}`,narrativeRole,
    `État initial : ${transformation.before}`,`Moment dramatique : ${transformation.moment}`,`Évolution visible : ${transformation.after}`,
    "","ENVIRONNEMENT COMPOSÉ — DÉCOR / PROFONDEUR",environment,`Composition : ${platformPolicy.composition}.`,
    "Construire un premier plan sombre, un plan principal narratif et un arrière-plan profond ou une ouverture. Les détails essentiels restent lisibles. Le regard doit sentir un avant et un après hors champ.",
    ...
    registers.fantastic?"Le fantastique doit sembler physiquement possible dans un long-métrage prestigieux ; pas de portail automatique, pas de cercle ésotérique, pas d’œil, pas de signes zodiacaux, pas de géométrie occulte, pas d’occultisme ou rituel.":"Aucun effet fantastique ajouté s’il n’est pas demandé.",
    `Branding futur : ${platformPolicy.brandingZone}. Cette zone reste intégrée au décor, jamais un aplat noir publicitaire.`,
    "","SORTIE BRUTE","Générer uniquement la photographie. Aucun texte, aucune lettre, aucun logo, aucun zèbre, aucun médaillon, aucun pictogramme, aucune URL, aucun hashtag."
...
module.exports={VERSION,resolveRegisters,explicitDemonstrationRequested,resolveVisualMode,transformationFrom,peoplePolicyFor,buildSceneIntent,auditSceneIntent,platformPolicyFor};
