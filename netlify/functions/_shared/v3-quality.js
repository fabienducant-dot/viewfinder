...
function assessQuality({contract,sceneIntent=null,analysis={},composition={},nonRedundancy={},cost={}}){
 ...
 const mode=sceneIntent?.mode||"legacy";
 const validation=sceneIntent?.validation||{};

 // Couche technique : aucune appréciation artistique ne peut masquer un défaut de fichier ou de marque.
 ...
 // Couche métier : les exigences littérales ne s'appliquent que lorsque le mode les demande.
 ...
 if(mode==="demonstration"||mode==="legacy"){
  ...
  if(contract.practitionerGenderRequired&&analysis.practitionerGender!==contract.practitionerGender)business.push("praticien_masculin_obligatoire_non_reconnu");
 }
 if(mode==="composite_fidelity"){
  ...
  if(analysis.foreignPersonPresent===true)business.push("identite_etrangere");
  if(contract.practitionerGenderRequired&&analysis.practitionerGender!==contract.practitionerGender)business.push("praticien_masculin_obligatoire_non_reconnu");
 }
 if((mode==="product_fidelity"||mode==="composite_fidelity")&&analysis.productFidelity!==true)business.push("fidelite_produit_non_verifiee");
 ...
 // Couche dramaturgique/DA : c'est ici que les anciens tests étaient trop faibles.
 ...
 if(paletteDrift>0.45)artistic.push("derive_palette_importante");else if(paletteDrift>0.15)warnings.push("derive_palette_moderee_corrigeable");
 ...
 const allErrors=[...technical,...business,...artistic,...nonRedundancyErrors];
 const visualHeuristics={
  heuristic:true,notCertainty:true,
  subjectImmediatelyUnderstandable:analysis.subjectMatchesRequest!==false,
  dramaticMoment:analysis.dramaticMomentPresent===true,
  transformationReadable:analysis.transformationReadable===true,
  cinematicPosterRead:analysis.cinematicPosterRead===true,
  threePlaneDepth:analysis.threePlaneDepth===true,
  emotionalStrength:Math.round(((analysis.dramaticMomentPresent?0.45:0)+(analysis.transformationReadable?0.35:0)+(composition.thumbnailImpact===false?0:.2))*10),
  sceneQuality:Math.round((Number(analysis.availableContrast||.7))*10),
  mobileReadability:composition.thumbnailImpact===false?4:8,
  officialLogoVisible:composition.logoIntegrity!==false&&composition.logoFringeDetected!==true,
  textHierarchy:composition.gazeHierarchyValid===false?4:8,
  sdzConsistency:paletteDrift>.45?3:(Number(analysis.genericSpaRisk||0)>.35?5:9),
  genericRisk:Math.max(Number(analysis.genericSpaRisk||0),Number(analysis.literalTreatmentSceneRisk||0)),
  overlapRisk:composition.protectedCollision===true?9:1,
  recommendedAction:technical.length?"recomposer_gratuitement":allErrors.length?"regenerer":"valider",
 };
 return {
  ok:allErrors.length===0,
  mode,
  technical:{ok:!technical.length,errors:technical},
  business:{ok:!business.length,errors:business},
  artistic:{ok:!artistic.length,errors:artistic},
  nonRedundancy:{ok:!nonRedundancyErrors.length,errors:nonRedundancyErrors,reason:nonRedundancy.reason||null},
  cost:{ok:cost.withinApprovedCeiling!==false,report:cost.report||null},
  visualHeuristics,warnings,
  preserveRawImage:true,preservePostText:true,
 };
}
