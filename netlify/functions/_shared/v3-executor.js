...
async function executeV3Pipeline({plan,rawImageBuffer,analyzeImage,composeImage,preserveRaw,brandComposition}){
 ...
 try{const imageBuffer=await composeImage(rawImageBuffer,finalization.layout,brandComposition);const manifest=imageBuffer?.compositionManifest||null;const measured=manifest||{};finalization={...finalization,compositionManifest:manifest,quality:assessQuality({contract:plan.contract,analysis:finalization.analysis,composition:{imageExists:true,paletteDrift:finalization.analysis.paletteDrift,contrastValid:finalization.analysis.availableContrast>=.25,gazeHierarchyValid:measured.hierarchyValid!==false,thumbnailImpact:finalization.analysis.density<.92,logoIntegrity:measured.logoWithinCanvas!==false,marginsValid:measured.marginsValid!==false,textWithinCanvas:measured.textWithinCanvas!==false&&measured.titleExact!==false&&measured.subtitleExact!==false,protectedCollision:measured.zonesDisjoint===false,logoRectangleOpaque:measured.logoRectangleOpaque===true}})};return {imageBuffer,brandComposited:true,finalization};}
