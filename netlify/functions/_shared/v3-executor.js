...
async function executeV3Pipeline({plan,rawImageBuffer,analyzeImage,composeImage,preserveRaw,brandComposition}){
 ...
 let finalization;
 try{const actual=await analyzeImage(rawImageBuffer,plan);finalization=finalizeV3(plan,actual,{imageExists:true,paletteDrift:actual.paletteDrift,contrastValid:actual.availableContrast>=.25,gazeHierarchyValid:true,thumbnailImpact:actual.density<.92});}
