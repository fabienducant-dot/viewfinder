...
function createHandler(openStore = openJobStore){
  ...
  try {
    ...
    if (job.status === "completed") {
      ...
      return {
        ...
        body: JSON.stringify({
          ...
          result: {
            resultUrl: job.resultKey ? `/.netlify/functions/get-image-result?jobId=${encodeURIComponent(jobId)}` : null,
    ...
    if (job.status === "queued" && Date.now() - job.createdAt > JOB_MAX_AGE_MS) {
      return { statusCode: 404, headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ok: false, jobId, status: "expired", error: "Travail en attente expiré (plus de 24h)" }) };
    }
...
exports.JOB_MAX_AGE_MS = JOB_MAX_AGE_MS;
exports.PROCESSING_TIMEOUT_MS = PROCESSING_TIMEOUT_MS;
