/* Fonction serveur Viewfinder — génération de texte.
   ...
   Pour ajouter un futur fournisseur "openai-compatible" (OpenRouter, Groq, Mistral...),
   rien à changer ici : le paramètre baseUrl suffit, tant qu'il utilise la même clé OPENAI_API_KEY
   ou une variable dédiée si besoin (voir commentaire plus bas). */
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (kind === "anthropic") {
      ...
      // "openai-compatible" : OpenAI par défaut. Un futur fournisseur compatible (OpenRouter, Groq, Mistral)
      // peut réutiliser cette même branche en passant sa propre baseUrl et sa propre variable de clé.
