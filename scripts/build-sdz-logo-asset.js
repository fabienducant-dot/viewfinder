...
async function main(){
 ...
 /* Même si l'œuvre originale touche le bord de l'icône source, le fichier destiné au compositeur
    reçoit une vraie gouttière alpha. Ainsi aucun pixel sombre du logo ne peut être confondu avec
    un fond ou créer le voile visible constaté en bas des affiches. */
 const cropPad=4,left=Math.max(0,minX-cropPad),top=Math.max(0,minY-cropPad),right=Math.min(width-1,maxX+cropPad),bottom=Math.min(height-1,maxY+cropPad);
 const extracted=await sharp(out,{raw:{width,height,channels:4}}).extract({left,top,width:right-left+1,height:bottom-top+1}).png().toBuffer();
 const alphaGutter=16;
 const clean=await sharp(extracted).extend({top:alphaGutter,bottom:alphaGutter,left:alphaGutter,right:alphaGutter,background:{r:0,g:0,b:0,alpha:0}}).png({compressionLevel:9,adaptiveFiltering:true}).toBuffer();
 ...
 let semiDark=0,edgeOpaque=0,opaquePixels=0,darkPixels=0,transparentPixels=0;
 for(let y=0;y<checkInfo.height;y++)for(let x=0;x<checkInfo.width;x++){
  ...
  if(a<=8)transparentPixels++;if(a>0)opaquePixels++;if(a>=245&&max<70)darkPixels++;if(a>0&&a<245&&max<90)semiDark++;
  if((x<2||y<2||x>=checkInfo.width-2||y>=checkInfo.height-2)&&a>16)edgeOpaque++;
 }
 const ratio=opaquePixels/(checkInfo.width*checkInfo.height),transparentRatio=transparentPixels/(checkInfo.width*checkInfo.height),darkRatio=darkPixels/Math.max(1,opaquePixels);
 if(semiDark!==0)throw new Error(`Halo semi-transparent sombre détecté : ${semiDark}`);
 if(edgeOpaque!==0)throw new Error(`La gouttière alpha n'est pas vierge : ${edgeOpaque} pixels opaques au bord.`);
 if(transparentRatio<.04)throw new Error(`Transparence extérieure insuffisante : ${transparentRatio}`);
 if(ratio<.05||ratio>.90)throw new Error(`Surface opaque anormale : ${ratio}`);
 ...
 console.log(JSON.stringify({output:path.relative(root,outputPath),preview:path.relative(root,previewPath),width:meta.width,height:meta.height,bytes:clean.length,sha256:sha,opaqueRatio:ratio,transparentRatio,darkInteriorRatio:darkRatio,semiDark,edgeOpaque},null,2));
}
